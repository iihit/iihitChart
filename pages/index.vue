  ...
  <div id="pageIndex">
    ...
    <div class="container">
      <div class="gridContainer">
        <MainGridData :upbitBitcoin="upbitBitcoin" :binanceBitcoin="binanceBitcoin" :dollar="dollar" :fearOrGreed="fearOrGreed" :trendingCoin="trendingCoin" :bitmexApiInfo="bitmexApiInfo" />
        <BitmexChatting :chat="chat" />
      </div>
    </div>
    <div class="container">
    ...
    <div class="container">
      <MainAltcoinMAStrategy :coinList="coinList" />
    </div>
    <!-- <div class="container">
      <RecommendedYoutube></RecommendedYoutube>
    </div> -->
  </div>
</template>
...
const chat = ref([])
const upbitBitcoin = ref(0)
const binanceBitcoin = ref(0)
const dollar = ref(0)
const fearOrGreed = ref(0)
const trendingCoin = ref({})
const bitmexApiInfo = ref({})
const coinList = ref([])
const tradePriceObject = useState()
...
onBeforeMount(async () => {
    ...
    const [
      ...
      pResponse10,
		...
		chat.value = pResponse1.value
    upbitBitcoin.value = pResponse2.value
    binanceBitcoin.value = pResponse3.value
    dollar.value = pResponse4.value
    fearOrGreed.value = pResponse5.value
    trendingCoin.value = pResponse6.value
    coinList.value = pResponse9.value
    // bitmexApiInfo.value = pResponse7.value

    console.log('pResponse10:', pResponse10.value.data.value)
    console.log('pResponse10:', pResponse10.value.pending.value)
...
// const lightweightChart = ref();
// const chart = ref();

// const getChartData = async () => {
  // const response = await useFetch('https://api.cryptowat.ch/markets/binance/btcusdt/ohlc')

  // const rawChartData = response.data.value.result['60']
...
// watchEffect(() => {
//   if (!lightweightChart.value) return;

//   chart.value = $LightWeightCharts.createChart(lightweightChart.value, {
//     width: 600,
//     height: 300,
//     rightPriceScale: {
//       scaleMargins: {
